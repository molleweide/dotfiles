...
function pickers_args {
    ...
    function help {
        cat <<-EOF >/dev/stderr
			    ...
			    USAGE:
			    $(basename "${BASH_SOURCE[2]}") [...<options>] <action> -- [...<picker-overrides>]
    ...
    if test "$(type -t "picker__$action")" = 'function'; then
        "picker__$action" "${options_fzf_helper_args[@]}" -- "${option_picker_program_args[@]}"
    else
